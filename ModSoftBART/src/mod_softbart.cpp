...
Hypers InitHypers(const mat& X, const uvec& group, double sigma_hat,
                  double alpha, double alpha_comp, double beta,
                  double gamma, double k, double width, double shape,
                  int num_tree, double alpha_scale, const vec& alpha_comp_scale, double alpha_shape_1,
                  double alpha_shape_2, double alpha_comp_shape_1, double alpha_comp_shape_2,
                  double tau_rate, double num_tree_prob,
                  double temperature) {

  int GRID_SIZE = 1000;
  
  Rcpp::Rcout << "InitHypers is called!" << std::endl;
  ...
  out.alpha_comp_vec = (double)alpha_comp * arma::ones<arma::vec>(out.num_groups); //each group has its hyperparameter
  ...
  // initialize omega_group based on group_to_vars
  out.omega_group.resize(out.num_groups);
  out.logomega_group.resize(out.num_groups);
  for(size_t g = 0; g < out.num_groups; g++){
    out.omega_group[g] = arma::ones<vec>(out.group_to_vars[g].size()) / ((double)(out.group_to_vars[g].size()));
    out.logomega_group[g] = log(out.omega_group[g]);
  }
...
int Hypers::SampleVar() const {
  ...
  if(group.max() + 1 < group.size()){
    //int var_idx = sample_class(arma::randu<arma::vec>(group_to_vars[group_idx].size()));
    var_idx = sample_class(omega_group[group_idx]);
  }
...
//   int group_idx = sample_class(s);
//   
//   // Check if group_idx is valid
//   if (group_idx < 0 || group_idx >= group_to_vars.size()) {
//     Rcpp::Rcerr << "Error: Invalid group_idx = " << group_idx << std::endl;
//     return -1;  // Return an error or handle it accordingly
//   }
//   
//   // Check if group_to_vars for this group is valid
//   if (group_to_vars[group_idx].empty()) {
//     Rcpp::Rcerr << "Error: group_to_vars[" << group_idx << "] is empty." << std::endl;
//     return -1;  // Return an error or handle it accordingly
//   }
//   
//   // Sample variable index
//   int var_idx = sample_class(group_to_vars[group_idx].size());
//   
//   // Ensure within-group sampling (if applicable)
//   if (group.max() + 1 < group.size()) {
//     if (group_idx >= omega_group.size()) {
//       Rcpp::Rcerr << "Error: omega_group size mismatch for group_idx = " << group_idx << std::endl;
//       return -1;  // Handle error
//     }
//     var_idx = sample_class(omega_group[group_idx]);
//   }
//   
//   // Return the sampled variable index
//   return group_to_vars[group_idx][var_idx];
// }
...
Rcpp::List do_soft_bart(const arma::mat& X,
  ...
  mat alpha_comp_vec = zeros<mat>(opts.num_save, hypers.s.size());
  ...
  for(int i = 0; i < opts.num_save; i++) {
    ...
    alpha_comp_vec.row(i) = trans(hypers.alpha_comp_vec);
    beta(i) = hypers.beta;
  ...
  out["s"] = s;
  out["alpha"] = alpha;
  out["alpha_comp_vec"] = alpha_comp_vec;
  out["beta"] = beta;
  out["gamma"] = gamma;
...
//Function to collect within group variable-level counts
void get_var_counts_by_variable(arma::uvec& counts, Node* node, const arma::uvec& group_vars) {
  if (!node->is_leaf) {
    // Check if the current node's variable is in the specified group
    auto it = std::find(group_vars.begin(), group_vars.end(), node->var);
    
    if (it != group_vars.end()) {
      // Get the index of the variable in the group
      size_t index_in_group = std::distance(group_vars.begin(), it);
      
      // Increment the count for this variable
      counts(index_in_group) += 1;
    }
    
    // Recursively traverse the left and right nodes
    get_var_counts_by_variable(counts, node->left, group_vars);
    get_var_counts_by_variable(counts, node->right, group_vars);
  }
}

arma::uvec get_var_counts_by_variable(std::vector<Node*>& forest, const Hypers& hypers, size_t group_idx) {
  // Access the variables for the specified group from hypers
  const arma::uvec& group_vars = hypers.group_to_vars[group_idx];
  
  // Initialize the count vector with size equal to the number of variables in the group
  arma::uvec counts = arma::zeros<arma::uvec>(group_vars.size());
  
  // Loop through each tree in the forest and calculate counts for the variables in this group
  for (Node* tree : forest) {
    get_var_counts_by_variable(counts, tree, group_vars);
  }
...
void UpdateOmegaGroup(std::vector<Node*>& forest, Hypers& hypers) {
  for (size_t g = 0; g < hypers.group_to_vars.size(); g++) {
    ...
    // Add the counts to the shape parameters for the Dirichlet distribution
    shape_up_vars = shape_up_vars + get_var_counts_by_variable(forest, hypers, g);
    
    // Sample from the Dirichlet using log-gamma trick
    for (int v = 0; v < num_vars_in_group; v++) {
      hypers.logomega_group[g][v] = rlgam(shape_up_vars(v));
    }
    
    // Normalize on the log scale and exponentiate
...
void Hypers::UpdateAlpha() {
  if(group.max() + 1 < group.size()){
    ...
    // update hyperparameters alpha_comp_vev
    for(int g = 0;  g < num_groups; g++){
      arma::vec logliks_comp = zeros<vec>(rho_propose.size());
      
      rho_loglik loglik_comp;
      loglik_comp.mean_log_s = mean(logomega_group[g]);
      loglik_comp.p = (double)omega_group[g].size();
      loglik_comp.alpha_scale = alpha_comp_scale(g);
      loglik_comp.alpha_shape_1 = alpha_comp_shape_1;
      loglik_comp.alpha_shape_2 = alpha_comp_shape_2;
      
      for(int i = 0; i < rho_propose.size(); i++) {
        logliks_comp(i) = loglik_comp(rho_propose(i));
      }
      
      logliks_comp = exp(logliks_comp - log_sum_exp(logliks_comp));
      double rho_up_comp = rho_propose(sample_class(logliks_comp));
      alpha_comp_vec(g) = rho_to_alpha(rho_up_comp, alpha_comp_scale(g));
    }
...
List SoftBart(const arma::mat& X, const arma::vec& Y, const arma::mat& X_test,
              ...
              double alpha, double alpha_comp, double beta, double gamma, double sigma,
              double shape, double width, int num_tree,
              double sigma_hat, double k, double alpha_scale, const arma::vec& alpha_comp_scale,
              double alpha_shape_1, double alpha_shape_2, double alpha_comp_shape_1, double alpha_comp_shape_2,
              double tau_rate,
  ...
  Hypers hypers = InitHypers(X, group, sigma_hat, alpha, alpha_comp, beta, gamma, k, width,
                             shape, num_tree, alpha_scale, alpha_comp_scale, alpha_shape_1,
                             alpha_shape_2, alpha_comp_shape_1, alpha_comp_shape_2, 
                             tau_rate, num_tree_prob, temperature);
...
Hypers::Hypers(Rcpp::List hypers) {
  ...
  alpha_comp_scale = as<arma::vec>(hypers["alpha_comp_scale"]);
  alpha_shape_1 = hypers["alpha_shape_1"];
  alpha_shape_2 = hypers["alpha_shape_2"];
  alpha_comp_shape_1 = hypers["alpha_comp_shape_1"];
  alpha_comp_shape_2 = hypers["alpha_comp_shape_2"];
  tau_rate = hypers["tau_rate"];
  ...
  alpha_comp_vec = (double)hypers["alpha_comp"] * arma::ones<arma::vec>(num_groups);
  ...
  // initialize omega_group based on group_to_vars
  omega_group.resize(num_groups);
  logomega_group.resize(num_groups);
  for(size_t g = 0; g < num_groups; g++){
    omega_group[g] = arma::ones<vec>(group_to_vars[g].size()) / ((double)(group_to_vars[g].size()));
    logomega_group[g] = log(omega_group[g]);
  }
...
arma::uvec Forest::get_counts() {
  return get_var_counts(trees, hypers); //trees is the forest.
}


Rcpp::List Forest::get_tree_counts() {
  if (hypers.group.max() + 1 == hypers.group.size()) {
    // Case when there are no sub-groups (i.e., variables are not grouped)
    for(int t = 0; t < hypers.num_tree; t++) {
      std::vector<Node*> tree;
      tree.resize(0);
      tree.push_back(trees[t]);
      tree_counts.col(t) = get_var_counts(tree, hypers);  // Get variable counts for each tree
    }
    
    return Rcpp::List::create(Rcpp::Named("tree_counts") = tree_counts);
  } else {
    // Case when variables are grouped
    arma::umat group_tree_counts = arma::zeros<arma::umat>(hypers.s.size(), hypers.num_tree);
    arma::umat var_tree_counts;
    
    // Get total number of variables across all groups
    int num_variables = 0;
    for (size_t i = 0; i < hypers.group_to_vars.size(); i++) {
      num_variables += hypers.group_to_vars[i].size();
    }
    var_tree_counts = arma::zeros<arma::umat>(num_variables, hypers.num_tree);
    
    // Loop over each tree and calculate both group and variable counts
    for (int t = 0; t < hypers.num_tree; t++) {
      std::vector<Node*> tree;
      tree.resize(0);
      tree.push_back(trees[t]);
      
      // Get group-level counts
      group_tree_counts.col(t) = get_var_counts(tree, hypers);
      
      // Get variable-level counts for each group in the forest
      for (size_t g = 0; g < hypers.group_to_vars.size(); g++) {
        // Get variable counts for group g
        arma::uvec var_counts_in_group = get_var_counts_by_variable(tree, hypers, g);
        
        // Place the counts into the appropriate rows of var_tree_counts, using group_to_vars for the row indices
        const arma::uvec& group_vars = hypers.group_to_vars[g];
        for (size_t v = 0; v < group_vars.size(); ++v) {
          int var_idx = group_vars[v];  // Get the actual variable index
          var_tree_counts(var_idx, t) = var_counts_in_group(v);  // Assign the count to the correct row
        }
      }
    }
    
    Rcpp::List counts;
    counts["Group_tree_counts"] = group_tree_counts;
    counts["Variable_tree_counts"] = var_tree_counts;
